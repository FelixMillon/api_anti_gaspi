...
class Modele 
{
	...
	public function selectAllProduitsWhere($id_entreprise)
	{
		if ($this->pdo != null)
		{
			$requete = "select p.*, c.libelle as lib from produit p, categorie_produit c where c.id_categorie = p.id_categorie and p.id_entreprise = :id_entreprise ;"; 
			$select = $this->pdo->prepare($requete);
	...
	public function Update_produit($tab, $where)
	{
		if ($this->pdo != null)
		{
			$donnees=array();
			...
			$chaineWhere = implode(" and ", $champs);
			$requete ="update produit set ".$chaineChamps."  where ".$chaineWhere;
	...
	public function Update_cat($tab, $where)
	{
		if ($this->pdo != null)
		{
			$donnees=array();
			...
			$chaineWhere = implode(" and ", $champs);
			$requete ="update categorie_produit set ".$chaineChamps."  where ".$chaineWhere;
